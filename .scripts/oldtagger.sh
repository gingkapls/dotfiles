	...
	then
        ...
        year=$(eyeD3 $song | awk -F "" '/recording date:/ {print $17 $18 $19 $20}')
			...
			artist=$(echo $song | awk -F ' - ' '{gsub(/.opus|.m4a|.mp3|.ogg|[(][^)]*[)]|[\[][^\]]*[\]]/,"",$NF); gsub(/^[ ]+|[ ]+$/, "", $NF); print $NF}')
	            eyeD3 -a "$artist" -t "$title" -A "$album" -n "$number" --recording-date "$year" --add-image "$thumbnail:FRONT_COVER" $song
		done)
	else if [ "$1" == "-no" ]
	then
		(IFS='
		...
		artist=$(echo $song | awk -F ' - ' '{gsub(/.opus|.m4a|.mp3|.ogg|[(][^)]*[)]|[\[][^\]]*[\]]/,"",$NF); gsub(/^[ ]+|[ ]+$/, "", $NF); print $NF}')
	        eyeD3 -a "$artist" -t "$title" -A "$album" --recording-date "$year" --add-image "$thumbnail:FRONT_COVER" $song
		done)
