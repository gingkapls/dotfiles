...
case "$1" in
	...
	;;
	"-p")
		grim -g "$(slurp -c "#FFFFFF" )" "$HOME/Pictures/shots/""$(date +"%F_%T")_"$app_id".png" && notify-send -i $(ls $HOME/Pictures/shots/* -drt | tail -n1) "Screenshot Captured" --app-name="screenshots" ## Partial screenshot 
	...
	;;
	*)
		grim "$HOME/Pictures/shots/""$(date +"%F_%T")_"$app_id".png" -c && notify-send -i $(ls $HOME/Pictures/shots/* -drt | tail -n1) "Screenshot Captured"  --app-name="screenshots" ## Normal screenshot  
		wl-copy < $(ls $HOME/Pictures/shots/* -drt | tail -n1)
	;;
esac
